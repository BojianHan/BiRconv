...
def create_network():
    ...
    # Comparison
    new_I = TT.tile(W_ic, (batch_size, 1)) * I
    C_row = TT.tile(new_I, (1, num_input)).reshape((batch_size, num_input, num_input))
    C_col = C_row.dimshuffle((0,2,1))
    new_B_c = TT.tile(B_c, (batch_size, 1, 1))
    new_W_ic = TT.tile(W_ic, (batch_size, 1, 1, 1))
    C = TT.nnet.relu(C_row * C_col + new_B_c)
...
np_W_ic = np.random.uniform(size=(num_input), low=-.1, high=.1)
